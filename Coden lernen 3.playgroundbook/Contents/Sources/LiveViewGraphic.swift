...
public class LiveViewGraphic {
    ...
    public func moveAndZap(to position: CGPoint, duration: Double = 0.0) {
        
        backingNode.run(.moveAndZap(to: position, duration: duration))
    }
    
    public func spinAndPop(after seconds: Double) {
        
        backingNode.run(.spinAndPop(after: seconds))
    }
