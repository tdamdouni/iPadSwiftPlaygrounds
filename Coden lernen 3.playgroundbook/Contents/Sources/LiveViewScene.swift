...
public class LiveViewScene: SKScene, ToolbarViewControllerDelegate, UIGestureRecognizerDelegate, GraphicAccessibilityElementDelegate, BackgroundAccessibilityElementDelegate {
    ...
    func handleMessage(message: Message) {
        ...
        case .moveAndZap(let id, let position, let duration):
            moveAndZap(id: id, to: position, duration: duration)

        case .spinAndPop(let id, let duration):
            spinAndPop(id: id, after: duration)
    ...
    func moveAndZap(id: String, to position: CGPoint, duration: Double) {
        
        DispatchQueue.main.async {
            guard let graphic = self.graphicsInfo[id] else { return }
            graphic.moveAndZap(to: position, duration: duration)
        }
    }
    
    func spinAndPop(id: String, after seconds: Double) {
        ...
        if showOverlays {
            if buttonBar == nil {
                if let view = overlayHost {
                    ...
                    let clearButton = ButtonBarButton.createANewButton(title: NSLocalizedString("Clear", comment:"Button to remove all the graphics from the scene"), target: self, action: #selector(clearButtonPressed))
