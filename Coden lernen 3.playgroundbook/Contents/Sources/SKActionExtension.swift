...
extension SKAction {
    ...
    public class func moveAndZap(to position: CGPoint, duration: Double = 0.0) -> SKAction {
        
        let moveAction = SKAction.move(to: position, duration: duration)
        moveAction.timingMode = .easeIn
        
        let soundAction = SKAction.audioPlayAction(fileNamed: "Warp Engineering 01.wav")
        
        let action = SKAction.sequence([soundAction, moveAction, .removeFromParent()])
