...
public struct SpeechTweak {
    ...
    public init(type: SpeechTweakType, effectFrom startValue: Number, to endValue: Number) {
        self.type = type
        
        let firstValue = startValue.int.clamped(to: Constants.userValueRange)
        let secondValue = endValue.int.clamped(to: Constants.userValueRange)
