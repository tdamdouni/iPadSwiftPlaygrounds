...
public func assessmentPoint() -> AssessmentResults {
    ...
    if !checker.didUseWhileLoop {
        hints[0] = NSLocalizedString("First, tap `while` in the shortcut bar to add your [outer loop](glossary://outer%20loop). Then add a condition so that the loop will run while your character isn't blocked.", comment:"Hint")
