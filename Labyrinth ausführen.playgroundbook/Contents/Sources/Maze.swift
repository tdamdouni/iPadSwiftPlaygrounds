...
public class Maze {
    ...
    var path: (coordinates: [Coordinate]?, hint: String?) {
        let hint = NSLocalizedString("There’s no path defined between the start and the goal. Plot a path through the maze by calling `maze.searchNeighbors(of:)` and inspecting its return value.", comment:"Hint message on where to start")
