...
public class LiveViewController: UIViewController, PlaygroundLiveViewSafeAreaContainer {
    ...
    @IBOutlet weak var searchCountDescriptionLabel: UILabel!

    @IBOutlet weak var searchCountLabel: UILabel!

    @IBOutlet weak var pathLengthDescriptionLabel: UILabel!
    ...
    public override func viewDidLoad() {
        ...
        searchCountDescriptionLabel.text = NSLocalizedString("NUMBER OF SEARCHES", comment: "The number of tiles the user's algorithm has searched")
        pathLengthDescriptionLabel.text = NSLocalizedString("LENGTH OF PATH", comment: "The length of the path the algorithm has found from the start to the goal")
    ...
    public func play(_ steps: [MazeRunner.Step]) {
        ...
        let plotPath = BlockOperation {
            ...
            if let coordinates = pathDetails.coordinates {
                ...
                self.pathLengthLabel.text = NSLocalizedString("∞", comment: "Symbol showing the algorithm never found a path (infinite length)")
            }
